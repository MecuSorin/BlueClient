...
public static void LogOnBBB(String... messages) {
			try {
				StringBuilder logMessage = new StringBuilder();
				logMessage.append("BBB ");
				for(String s:messages) {
					String ss = s;
					if( null == s) ss = "null";
					if(0 == s.length()) ss = "empty";
					logMessage.append(ss);
				}

				Log.w("BBB", logMessage.toString() );
			} catch(Exception e) {}
		}
	...
	private class ConnectedThread extends Thread {
		...
		public ConnectedThread(BluetoothSocket socket, String socketType) {
			...
			mmOutStream = tmpOut;
			if(null != mmInStream) {
				BluetoothSerialService.LogOnBBB("Created the bluetooth input and output sockets");
			}
		}
		...
		public void run() {
			...
			while (true) {
				try {
					...
					BluetoothSerialService.LogOnBBB("received: ", data);
		...
		public void write(byte[] buffer) {
			try {
				...
				BluetoothSerialService.LogOnBBB("Sended message to bluetooth");
